"""Ingest Liander open-data street baselines into customer spatial context."""
...
def run(session: Session, seed: int | None = None, csv_path: str | None = None) -> dict:
    from gridtrace_worker.config import get_worker_config

    cfg = get_worker_config()
    resolved_seed = cfg.demo_seed if seed is None else seed
    ingest = DutchEnergySpatialIngestion(csv_path=csv_path)
